...
2. **Step 2: Gemini 初步分析** - 参考字体样式库 (textstyle.pdf) 生成初步的 fabric.js JSON（背景、形状、文字样式、字体类型）
...
### Step 2: Gemini 初步分析 + 字体库参考

Gemini 分析图片时，会同时参考 **字体样式库 (textstyle.pdf)** 来精确识别字体类型：

**输入：**
- 原图
- 字体样式参考文件 (`image/textstyle.pdf`)
...
- 形状识别（矩形、圆形等）
- 文字样式识别（粗体、斜体、颜色）
- **字体类型识别** - 参考 textstyle.pdf 字体库精确匹配（Serif、Sans-serif、Cursive 等）

> 💡 **字体库参考**：`image/textstyle.pdf` 包含常用字体的样式指南，Gemini 会对比图片中的文字与字体库来判断字体类型。
